...
namespace DAL
{
    class Activity
    {
        ...
        public Model.Activity GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVerify='审核通过' and recommend='是' and endTime>=new() order by recommendTime desc limit 1";
        ...
        public Model.Activity GetEnd()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from activity where activityVerify='审核通过' and endTime<new() order by endTime desc limit 1";
                return cn.QueryFirstOrDefault<Model.Activity>(sql);
            }
        }
        ...
        public int Update(Model.Activity activity)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
        ...
        public int UpdateImg(Model.Activity activity)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
        ...
        public int UpdateVerify(Model.Activity activity)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update activity set activityVerify=@activityVerify where activityid=@acitvityId";
                return cn.Execute(sql, activity);
            }
        }
